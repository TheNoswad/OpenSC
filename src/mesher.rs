...
use crate::{blocksdata::BlocksData, chunks32h::Chunk};

const TEXTURE_ATLAS_SIZE: u8 = 16;
const SLOT_WIDTH: f32 = 1.0 / TEXTURE_ATLAS_SIZE as f32;

pub fn generate_chunk_mesh(chunk: &Chunk, blocksdata: &BlocksData) -> Mesh {
        ...
        for x in 0..16 {
            for y in 0..256 {
                ...
                if block != 0 {
                    ...
                    let mut block_mesh = generate_block_mesh(
                        x,
                        y,
                        z,
                        block,
...
pub fn generate_block_mesh(
    ...
    block: u16,
    indices_count: u32,
    face_occulsion: [bool; 6],
    blocksdata: &BlocksData,
) -> (Vec<Vec3>, Vec<u32>, Vec<Vec2>, u32) {
    ...
    if blockid <= 254 {
        let texturelocation = &blocksdata.Blocks[blockid as usize].TextureLocation;

        texcoordsx = vec2(
        texturelocation[0] / TEXTURE_ATLAS_SIZE as f32,
        (texturelocation[0] / TEXTURE_ATLAS_SIZE as f32) + SLOT_WIDTH,
        );
    
        texcoordsy = vec2(
        texturelocation[1] / TEXTURE_ATLAS_SIZE as f32,
        (texturelocation[1] / TEXTURE_ATLAS_SIZE as f32) + SLOT_WIDTH,
        );
    }
    else {
        texcoordsx = vec2(0.0, 1.0);
        texcoordsy = vec2(0.0, 1.0);
    }
    ...
    // Check if the block id is defined in the list
    // if blockid as usize > blocksdata.len() {
    //     // Block id was past the len of the vec of values
    //     texcoordsx = vec2(0.0, 1.0);
    //     texcoordsy = vec2(0.0, 1.0);
    // } else {
    //     // The block id is within the range
    //     // Now check if a texture slot is defined
    //     match blocksdata[blockid as usize].DefaultTextureSlot {
    //         Some(texture_index) => {
    //             //let texture_index = blocksdata[blockid as usize].DefaultTextureSlot;

    //             let texture_slot_x: f32 = (texture_index % 16).into();
    //             let texture_slot_y: f32 = (texture_index / 16).into();

    //             texcoordsx = vec2(
    //                 texture_slot_x / TEXTURE_ATLAS_SIZE as f32,
    //                 (texture_slot_x / TEXTURE_ATLAS_SIZE as f32) + SLOT_WIDTH,
    //             );

    //             texcoordsy = vec2(
    //                 texture_slot_y / TEXTURE_ATLAS_SIZE as f32,
    //                 (texture_slot_y / TEXTURE_ATLAS_SIZE as f32) + SLOT_WIDTH,
    //             );
    ...
    // let texture_slot_x = texture_index % 16;
    // let texture_slot_y = texture_index / 16;

    // let texcoordsx = vec2(
    //     (texture_slot_x / TEXTURE_ATLAS_SIZE).into(),
    //     (texture_slot_x + 1 / TEXTURE_ATLAS_SIZE).into(),
    // );

    // let texcoordsy = vec2(
    //     (texture_slot_y / TEXTURE_ATLAS_SIZE).into(),
    //     (texture_slot_y + 1 / TEXTURE_ATLAS_SIZE).into(),
    // );
        ...
        match i {
            ...
            1 => {
                if face_occulsion[0] != true {
                    indices.append(&mut vec![
                        0 + indices_cnt_offset,
                        1 + indices_cnt_offset,
                        5 + indices_cnt_offset,
                        5 + indices_cnt_offset,
                        4 + indices_cnt_offset,
                        0 + indices_cnt_offset,
                    ]);
                    //indices_count += 1
                }
            ...
            2 => {
                if face_occulsion[1] != true {
                    indices.append(&mut vec![
                        2 + indices_cnt_offset,
                        3 + indices_cnt_offset,
                        7 + indices_cnt_offset,
                        7 + indices_cnt_offset,
                        6 + indices_cnt_offset,
                        2 + indices_cnt_offset, // the last one was three... idk maybe it works
                    ]);
                    //indices_count += 1
                }
            }
